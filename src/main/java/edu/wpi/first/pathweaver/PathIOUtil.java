...
public final class PathIOUtil {
	 ...
	 *
	 * @param fileLocation
	 *            the directory and filename to write to
	 * @param path
	 *            Path object to save
	 *
	...
	public static boolean export(String fileLocation, Path path) {
		try (BufferedWriter writer = Files.newBufferedWriter(Paths.get(fileLocation + path.getPathName()));

				CSVPrinter csvPrinter = new CSVPrinter(writer,
						CSVFormat.DEFAULT.withHeader("X", "Y", "Tangent X", "Tangent Y", "Fixed Theta", "Name"))) {
	...
	public static Path importPath(String fileLocation, String fileName) {
		try (Reader reader = Files.newBufferedReader(java.nio.file.Path.of(fileLocation, fileName));
			...
			for (CSVRecord csvRecord : csvParser) {
				Point2D position = new Point2D(Double.parseDouble(csvRecord.get("X")),
						Double.parseDouble(csvRecord.get("Y")));
				Point2D tangent = new Point2D(Double.parseDouble(csvRecord.get("Tangent X")),
