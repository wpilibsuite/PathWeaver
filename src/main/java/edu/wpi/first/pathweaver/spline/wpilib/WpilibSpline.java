...
/**
 * A WpilibSpline interfaces with Wpilib to calculate splines.
 */
public class WpilibSpline extends AbstractSpline {
	...
	public boolean writeToFile(java.nio.file.Path path) {
		...
		try (BufferedWriter writer = Files.newBufferedWriter(path);
				CSVPrinter csvPrinter = new CSVPrinter(writer,
						CSVFormat.DEFAULT.withHeader("X", "Y", "Tangent X", "Tangent Y"))) {
			for (Waypoint waypoint : waypoints) {
				csvPrinter.printRecord(waypoint.getX(), waypoint.getY(), waypoint.getTangentX(),
						waypoint.getTangentY());
			}
			csvPrinter.flush();
			return true;
	...
	private static QuinticHermiteSpline[] getQuinticSplinesFromWaypoints(Waypoint[] waypoints) {
		...
		for (int i = 0; i < waypoints.length - 1; i++) {
			...
			// double p0Magnitude = Math.pow(p0.getTangentX(), 2) +
			// Math.pow(p0.getTangentY(), 0);
			//
			// double p1Angle = Math.atan2(p1.getTangentY(), p1.getTangentX());
			// double p1Magnitude = Math.pow(p1.getTangentX(), 2) +
			// Math.pow(p1.getTangentY(), 0);
			...
			splines[i] = new QuinticHermiteSpline(xInitialVector, xFinalVector, yInitialVector, yFinalVector);
		}
