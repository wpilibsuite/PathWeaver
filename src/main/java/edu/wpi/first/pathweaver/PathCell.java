...
public class PathCell extends TextFieldTreeCell<String> {
	...
	/**
	 * Creates PathCell, a TreeCell object that can be dragged and used as a drag
	 * target.
	 *
	 * @param validDropTarget
	 *            if this item should allow drag over and drag drop.
	 * @param validation
	 *            a bifunction that checks if renaming the cell is valid
	 */
	...
	private void setupDragOver() {
		...
		cell.setOnDragOver(event -> {
			...
			if (item != null) {
				...
				if (parent != null && event.getDragboard().hasString()) {
